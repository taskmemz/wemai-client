...
| `ws_client.py` | TCP 长连接，4 字节长度前缀 JSON 协议，客户端心跳 + TCP keepalive 加速 |
...
7. **好友请求**：随机间隔（2-10 分钟，高斯分布）检查新好友请求 → 已添加/已过期的跳过 → 待验证的推送给 Adapter。支持运行统计（检查次数、发现次数、平均间隔）
...
| **好友检查反检测** | 随机间隔 2-10 分钟 + 运行统计日志，避免固定频率触发风控 |
| **客户端心跳** | 30 秒无活动自动发 ping，TCP keepalive 10s/3s/3 次，保持连接不踢 |
